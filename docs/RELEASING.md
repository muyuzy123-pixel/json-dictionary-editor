...
归档步骤需要 Python 3 标准库（zipfile）；工具链由维护者安装，不自动下载。
...
3. Windows 显式指定已核对的 LLVM-MinGW 20260616，运行交叉构建，再调用 `windows/scripts/package_preview.sh`。记录链接映射及 verbose linker 的 Loaded 归档记录，装入实际需要的运行库许可。普通 Windows 构建无需 Mac 图标生成器。
